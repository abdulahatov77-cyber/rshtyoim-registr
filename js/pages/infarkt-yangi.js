...
const InfarktYangiPage = {
  ...
  // Router parametrlaridan yuboruvchi bemorni o'qiб _data ni to'ldiradi
  async _qabulManbasi() {
    const p = Router._params || {};
    InfarktYangiPage._manba = null;
    if (!p.qabul_kt || p.qabul_turi !== 'infarkt') return null;
    const src = await DB.yuborilganBemor(p.qabul_kt, 'infarkt').catch(() => null);
    if (!src) return null;
    ...
    return `
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
        ...
        ${this.field('qabul_vaqt','Bemor qabul qilingan sana va vaqt',`<div class="flex gap-2">
            <input id="qabul_sana" type="date" class="form-input" max="${new Date(Date.now()+5*3600000).toISOString().slice(0,10)}" value="${d.qabul_vaqt?.slice(0,10)||''}" onchange="InfarktYangiPage.onQabulVaqtChange()"/>
            <input id="qabul_soat" type="time" class="form-input" value="${d.qabul_vaqt?.slice(11,16)||''}" onchange="InfarktYangiPage.onQabulVaqtChange()"/>
          </div>`,true)}
        ...
        <div id="tez-yordam-div" style="display:${d.murojaat_yoli==='Tez tibbiy yordam bilan'?'block':'none'}">
          ${this.field('tez_yordam_kelgan_vaqt','Tez yordam yetib keldi (vaqt)',`
            <div class="flex gap-2">
              <input id="tez_yordam_kelgan_sana" type="date" class="form-input" max="${new Date(Date.now()+5*3600000).toISOString().slice(0,10)}" value="${d.tez_yordam_kelgan_vaqt?Utils.formatDateInput(d.tez_yordam_kelgan_vaqt).slice(0,10):''}" onchange="InfarktYangiPage.onTezYordamChange()"/>
              <input id="tez_yordam_kelgan_soat" type="time" class="form-input" value="${d.tez_yordam_kelgan_vaqt?Utils.formatDateInput(d.tez_yordam_kelgan_vaqt).slice(11,16):''}" onchange="InfarktYangiPage.onTezYordamChange()"/>
  ...
  onFuqarolikChange(val) {
    ...
    if (val === 'Chet el') {
      InfarktYangiPage._data.yashash_viloyat = '';
      InfarktYangiPage._data.yashash_tuman = '';
    } else {
  ...
  onYashashViloyatChange(val) {
    InfarktYangiPage._data.yashash_viloyat = val;
    InfarktYangiPage._data.yashash_tuman = '';
  ...
  validateStep() {
    ...
    // Puls chegarasi (20–300) va qon bosimi formati (masalan 140/90)
    if (this._step === 2 && valid) {
      const puls = parseInt(this._data.puls);
      if (this._data.puls && (isNaN(puls) || puls < 20 || puls > 300)) {
        valid = false;
    ...
    if (this._step === 3) {
      // Shifokor telefoni — kamida 7 raqam bo'lishi kerak
      if (valid && this._data.shifokor_tel) {
