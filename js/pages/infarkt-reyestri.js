// ==================== INFARKT REYESTRI SAHIFASI ====================
const InfarktReyestriPage = {
  _viloyat: null,
  _currentData: [],
  ...
  async loadStats() {
    ...
    try {
      const data = await DB.getRegistryStats('infarkt', this._viloyat);
    ...
    let html = `
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ${this._viloyat && (this._profile?.role === 'admin' || this._profile?.role === 'super_admin') ? `<button class="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm" onclick="InfarktReyestriPage.goBack()">${icon('arrow-left', 20)}</button>` : ''}
          <div>
            <h2 class="text-xl font-black text-slate-800 leading-none">${title}</h2>
            <p class="text-xs font-bold text-slate-400 uppercase mt-1">Infarkt patologiyasi bo'yicha</p>
          </div>
        </div>
        <div class="flex gap-2">
          <button class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition-all shadow-sm" onclick="InfarktReyestriPage.refreshStats()">
            ${icon('refresh-cw', 14)} Yangilash
          </button>
          <button class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200" onclick="InfarktReyestriPage.exportData()">
          ...
          <table class="w-full border-collapse">
            ...
            <tbody class="divide-y divide-slate-50">
              ...
              ${data.map(r => {
                ...
                return `
                  <tr class="hover:bg-slate-50/80 transition-colors cursor-pointer group" data-name="${esc(r.name)}" onclick="InfarktReyestriPage._rowClick(this)">
                    <td class="px-6 py-5">
  ...
  exportData() {
    const filename = this._viloyat ? `infarkt_${this._viloyat}.csv` : 'infarkt_viloyatlar.csv';
