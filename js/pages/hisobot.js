...
const HisobotPage = {
  ...
  renderUI(user) {
    ...
    inner.innerHTML = `
      ...
      <div class="h-card">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
          <div>
            <h3 class="h-title !mb-1">${icon('file-text', 18)} Infarkt va insult bemorlari, muolaja guruhlari va letallik xisoboti</h3>
          </div>
          <div class="flex flex-wrap items-center gap-2">
            ${isSuperAdmin ? `
  ...
  renderReport(infs, ins, kuzatuv, from, to, ageLabel = '', locationLabel = '') {
    ...
    const mskt = ins.filter(p=>Utils.msktDone(p.mskt)).length;
    const msktDavol = ins.filter(p=>Utils.msktDone(p.mskt)&&p.status==='chiqarildi').length;
    const msktVafot = ins.filter(p=>Utils.msktDone(p.mskt)&&p.status==='vafot').length;
    ...
    el.innerHTML = `
      ...
      ${(() => {
        ...
        <div class="h-card !p-0 overflow-hidden mt-6">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2">
            <div class="border-r border-slate-100 flex flex-col">
              <div class="p-3 bg-slate-50 border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase sticky top-0 z-10">O'tkazilgan muassasa</div>
              <div style="max-height:400px;overflow-y:auto">
                <table class="w-full text-sm">
                  ${tableHead([{label:'Muassasa'},{label:'Infarkt',align:'center'},{label:'Insult',align:'center'},{label:'Jami',align:'center'}])}
